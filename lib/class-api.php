...
class Selz_API
{
    ...
    public function connect()
    {
        ...
        if ($this->register_client()) {
            ...
            $args = array(
                ...
                'state' => $this->generate_auth_state()
            );
        ...
        if (isset($_GET['page']) && $_GET['page'] == $this->slug) {
            if (isset($_GET['error']) && $_GET['error'] != '') {
                ...
                if (isset($_GET['state']) && $_GET['state'] != '') {
                    if ($_GET['state'] != $this->get_auth_state()) {
                        return new WP_Error('unknown', 'Auth state is invalid');
                    }
                } else {
                    return new WP_Error('unknown', 'Auth state is missing');
                }

                // State is valid, now exchange code with token
    ...
    public function generate_auth_state()
    {
        // Generate a random state parameter for CSRF security
        $state = bin2hex(random_bytes(50));
        update_option($this->slug . '_auth_state', $state);
        return $state;
    }

    public function get_auth_state()
    {
        // Fetch state and remove from DB (can only be used once)
        $state = get_option($this->slug . '_auth_state');
        delete_option($this->slug . '_auth_state');
        return $state;
    }
